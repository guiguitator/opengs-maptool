...
def command_processing(context: ApplicationContext, message: Message) -> Message:
    """
    ...
    match command:
        case 'clear':
            message_text, message_type = _clear_command(context, arguments)
        
        case 'discord':
            message_text, message_type = _discord_command(context, arguments)

        case 'github':
            message_text, message_type = _github_command(context, arguments)

        case 'history':
            message_text, message_type = _history_command(context, arguments)

    console_controller = ConsoleController(context)
...
def _clear_command(context: ApplicationContext, arguments: list) -> tuple[str, MessageType]:
    if len(arguments) != 0:
...
def _discord_command(context: ApplicationContext, arguments: list) -> tuple[str, MessageType]:
    if len(arguments) != 0:
        return _unknow_arguments_response(arguments)
...
def _github_command(context: ApplicationContext, arguments: list) -> tuple[str, MessageType]:
    if len(arguments) != 0:
        return _unknow_arguments_response(arguments)
...
def _history_command(context: ApplicationContext, arguments: list) -> tuple[str, MessageType]:
    if len(arguments) != 0:
        return _unknow_arguments_response(arguments)
